...
export default function PlaygroundPage() {
  ...
  const handleQuery = useCallback(
    async (q: string) => {
      ...
      try {
        const [insecureRes, secureRes] = await Promise.all([
          queryRAGApi(q, currentUser.id, 'insecure', controller.signal),
          queryRAGApi(q, currentUser.id, 'secure', controller.signal),
        ]);

        const insecure = insecureRes.response;
        const secure = secureRes.response;
        setLlmMode(insecureRes.llmMode);
