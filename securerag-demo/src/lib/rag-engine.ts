...
/**
 * Call the server-side RAG API endpoint.
 * Falls back to local mock functions if the API returns { fallback: true }
 * or if the fetch fails (e.g. network error, no API key).
 */
export async function queryRAGApi(
  ...
  signal?: AbortSignal
): Promise<{ response: RAGResponse; llmMode: boolean }> {
  try {
    const res = await fetch('/api/rag', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query, userId, mode }),
      signal,
    });
    ...
    if (data.fallback) {
      // Server says no API key — use mock
      const mockFn = mode === 'insecure' ? simulateInsecureRAG : simulateSecureRAG;
      return { response: await mockFn(query, userId, signal), llmMode: false };
    }

    return { response: data as RAGResponse, llmMode: true };
  } catch (error) {
    // If aborted, re-throw so the UI handles it
